...
class Invader:
    ...
    def step(self, action):
        ...
        elif self.counter == 18000:
            done = True
            sys.stdout.write("Time over!\n")
    ...
    def _get_observation(self):
        ...
        beam_check_range = np.array([64, 64, 30, 24])  # L,R,U,D(学習に使う範囲を指定)
    ...
    def update(self):
        ...
        if self.game_state == PLAY:
            ...
            for alien in self.aliens:
                alien.shoot_extra_beam(self.player.rect.center[0], 64, 2)
                #エイリアンの攻撃間隔
    ...
    def collision_detection(self):
        ...
        for alien in alien_collided.keys():
            ...
            num_aliens = len(self.aliens)
            #print(num_aliens)
            #エイリアンが残り8体以下になったら2体倒すごとにスピードup
            if num_aliens <= 8:
                if num_aliens % 2 == 0:
                    for alien in self.aliens:
                        #print("スピードup")
                        if alien.speed < 0:
                            ...
                            alien.speed += 1
            #エイリアンを15体倒すごとにスピードup
            if (50 -num_aliens) % 15 == 0:
                #エイリアンの撃破時エイリアンのスピードup
                for alien in self.aliens:
                    #print("スピードup")
                    if alien.speed < 0:
                        alien.speed -= 1
...
class Alien(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, wave):
        ...
        #self.prob_beam = 0.03  # ビームを発射する確率
    ...
    def shoot_extra_beam(self, target_x_pos, border_dist, rate):
        ...
        #print(self.prob_beam * rate and abs(self.rect.center[0] - target_x_pos))
        if random.random() < self.prob_beam*rate and abs(self.rect.center[0] - target_x_pos) < border_dist:
            if self.bfrag == 0:
                Beam(self.rect.center)
                self.bfrag = 1
